...
export function PasscodeScreen({ navigation, route }) {
  ...
  switch (screenMode) {
    ...
    case LOCAL_AUTH_SCREEN_MODE.REPEAT_PASSCODE:
      title = "Введите ваш пароль еще раз";
      iconName = "create";
      break;
  ...
  if (attemptsCount > 0) {
    title = `Неверный пароль. У вас осталось ${maxAttemptsCount - attemptsCount} попыток`;
  }

  if (attemptsCount >= maxAttemptsCount) {
    title = `Неверный пароль. У вас не осталось попыток`;
  }
