...
export function ReceiveScreen({ navigation }) {
  ...
  const handleCopyAddress = () => {
    setString(`Публичный ключ для получения ${putSymbol}: ${address}`);
    Alert.alert("Адрес скопирован");
  };
  ...
  const handleShare = async () => {
    try {
      await qrRef.current.toDataURL(async (data) => {
        await Share.share({
          message: `Публичный ключ для получения ${putSymbol}: ${address}\n © DeMo BANK`,
          title: "Мой публичный адрес",
        });
      });
    } catch (e) {
      console.error(e);
  ...
  return (
    ...
    >
      ...
      >
        {`Отправляйте только ${putSymbol} на этот адресс. Отправка другого put может привести к потери отправляемых средств без возможности их возврата`}
      </CustomText>
      ...
      >
        <Dialog.Title>
          <CustomText>Введите сумму</CustomText>
        </Dialog.Title>
        <Dialog.Input
